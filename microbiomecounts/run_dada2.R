...
#  FUNCTION VERSION  -----------------------------------------------------
# -----------------------------------------------------------------------
run_dada2 <- function(biop, inputfile, threads, outdir, debug_mode = FALSE) {

  ## 0) DEBUG HANDLER ----------------------------------------------------
  if (debug_mode) {
    options(error = recover)   # or just set browser() in hot spots
  }

  ## 0) THREADS / REGISTRATION ------------------------------------------
  ...
  SIZE_THRESHOLD_GB <- 20  # Chunk if >20 GB
  CHUNK_LINES       <- 4e6 # (unused; placeholder)
  ...
  for (fnF in fnF_list) {
    ...
    if (fsize_gb > SIZE_THRESHOLD_GB) {
      ...
      for (fc in filtered_chunks) {
        writeLines(readLines(fc), outF, useBytes = TRUE, append = TRUE)
      }
  ...
  sample.names <- sub("^filtered_([^_]+)(?:_1)?\\.fastq$", "\\1", basename(filts), perl = TRUE)

  ## If manifest changed, clear old model
  if (file.exists(smp_chk)) {
    saved_sample.names <- readRDS(smp_chk)
    if (!identical(sort(saved_sample.names), sort(sample.names))) {
      message("Sample list changed; clearing old error model.")
  ...
  if (file.exists(err_chk) && file.exists(smp_chk)) {
    ...
    if (anyDuplicated(sample.names)) stop("Duplicate sample names detected!")
  } else {
    message("Learning error rates")
    names(filts) <- sample.names
    if (anyDuplicated(sample.names)) stop("Duplicate sample names detected!")

    err <- learnErrors(filts, nbases = 1e8,
                       multithread = TRUE, randomize = TRUE, verbose = VERBOSE)
    saveRDS(err, err_chk)
  ...
  if (file.exists(dds_chk)) {
    ...
    for (sam in sample.names) {
      ...
      dds[[sam]] <- dada(derep, err = err,
                         multithread = TRUE, verbose = VERBOSE)
    }
    saveRDS(dds, dds_chk)
  }
  ...
  if (!file.exists("rdp_19_toGenus_trainset.fa.gz")) {
    stop("RDP taxonomy file not found (rdp_19_toGenus_trainset.fa.gz)")
  }
  ...
  if (file.exists(tax_chk)) {
    ...
    tax  <- assignTaxonomy(
      seqs, "rdp_19_toGenus_trainset.fa.gz",
      multithread = TRUE, verbose = VERBOSE
    )

    if (!"Kingdom" %in% colnames(tax)) {
      stop("Taxonomy object invalid (no 'Kingdom' column)")
    }
    saveRDS(tax, tax_chk)
  }
...
if (!interactive()) {
  ...
  raw_args   <- setdiff(raw_args, "--debug")

  if (length(raw_args) != 4) {
    stop("Usage: run_dada2.R <bioproject> <inputfile> <threads> <outdir> [--debug]")
  }

  run_dada2(
    biop       = raw_args[1],
    inputfile  = raw_args[2],
    threads    = as.integer(raw_args[3]),
    outdir     = raw_args[4],
