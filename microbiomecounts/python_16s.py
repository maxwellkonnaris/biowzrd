...
def main():
    ...
    # If user only wants to submit per-bioproject jobs, skip all processing
    if hasattr(args, 'submit_only') and args.submit_only:
        import pandas as pd
        # Read subset CSV directly
        df = pd.read_csv(args.input_file, sep=delim, dtype=str)
        bioprojects = df["Bioproject"].unique().tolist()
        # Ensure output directories exist
        setup_directories()
        # Submit one SLURM job per Bioproject
    ...
    logger.info(f"Using {args.num_workers} workers for non-DADA2 parallel steps")
